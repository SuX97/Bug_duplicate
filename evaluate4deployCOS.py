...
def calculateCOSDistance(predA, predB):
    predA = normalize(predA, norm='l2', axis=1)
    predB = normalize(predB, norm='l2', axis=1)
    distances = np.sum(predA*predB, keepdims=False, axis=-1)
    return distances 
...
infoRetrivalModel = load_model(modelFileName)
#############################################

############## run predict ###############
featuresA = infoRetrivalModel.predict(x=[clsTestA], batch_size=1024, verbose=1)
featuresB = infoRetrivalModel.predict(x=[clsTestB], batch_size=1024, verbose=1)

distances = calculateCOSDistance(featuresA, featuresB)

fpCount = 0.0
tpCount = 0.0
for i in range(0, len(clsTestLabel), 2):
    if (clsTestLabel[i] == 0) and (distances[i] > FP_THRESHOLD):
        fpCount += 1
        print("[FP] X=[%s, %s] Y=%s, Predicted=%s" % (testDataList[i/2][0], testDataList[i/2][1], clsTestLabel[i], distances[i]))
    elif (clsTestLabel[i] == 1) and (distances[i] > FP_THRESHOLD):
        tpCount += 1
        print("[TP] X=[%s, %s] Y=%s, Predicted=%s" % (testDataList[i/2][0], testDataList[i/2][1], clsTestLabel[i], distances[i]))
    else:
        print("[EE] X=[%s, %s] Y=%s, Predicted=%s" % (testDataList[i/2][0], testDataList[i/2][1], clsTestLabel[i], distances[i])) 
    if (clsTestLabel[i+1] == 0) and (distances[i+1] > FP_THRESHOLD):
        fpCount += 1
        print("[FP] X=[%s, %s] Y=%s, Predicted=%s" % (testDataList[i/2][0], testDataList[i/2][2], clsTestLabel[i+1], distances[i+1]))
    elif (clsTestLabel[i+1] == 1) and (distances[i+1] > FP_THRESHOLD):
        tpCount += 1
        print("[TP] X=[%s, %s] Y=%s, Predicted=%s" % (testDataList[i/2][0], testDataList[i/2][2], clsTestLabel[i+1], distances[i+1]))
    else:
        print("[EE] X=[%s, %s] Y=%s, Predicted=%s" % (testDataList[i/2][0], testDataList[i/2][2], clsTestLabel[i+1], distances[i+1])) 
